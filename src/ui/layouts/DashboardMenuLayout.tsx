...
import { NotificationPopover } from "@/ui/organisms/dashboard/common/NotificationPopover";

interface DashboardMenuLayoutProps {
  children: React.ReactNode;
}

export function DashboardMenuLayout({ children }: DashboardMenuLayoutProps) {
  const [title, setTitle] = useState(""); // Default title
  const [showLogoutDialog, setShowLogoutDialog] = useState(false);
  const [showUserMenu, setShowUserMenu] = useState(false);
  // showNotifications state removed
    ...
    <DashboardContext.Provider value={{ title, setTitle }}>
      ...
      <div className="flex min-h-screen w-full flex-col">
        ...
        <header className="sticky top-0 z-40 w-full border-b bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60">
          <div className="w-full flex h-16 items-center justify-between px-4 md:px-6 relative">
            {/* TradeNotz Brand - Left side */}
            <Link
              to="/"
              className="flex-shrink-0 cursor-pointer hover:opacity-80 transition-opacity"
            >
              <span className="text-xl font-bold text-primary">TradeNotz</span>
            </Link>
              ...
              {menuList.map((item) => {
                ...
                return (
                  <Link
                    key={item.path}
                    to={item.path}
                    className={`flex items-center gap-2 px-4 py-2 rounded-md text-sm font-medium transition-colors whitespace-nowrap ${isActive
                      ? "bg-accent text-accent-foreground hover:bg-blue-100 hover:text-blue-700"
            ...
            <div className="flex items-center gap-3 relative flex-shrink-0">
              ...
              >
                <Avatar className="h-10 w-10 border-2 border-border bg-white">
                  <AvatarImage
                    src={profileImageUrl}
                      ...
                      <CardContent className="pt-0 pb-6">
                        <div className="flex flex-col items-center space-y-4">
                          ...
                          <div className="relative">
                            <Avatar className="h-24 w-24 border-4 border-border bg-white">
                              <AvatarImage
                                src={profileImageUrl}
                                alt={user?.fullName || "User"}
                              />
        ...
        <main className="flex-1 w-full">
          ...
          {user && !(user as any).isAgentVerified && (
            <div className="bg-amber-50 border-l-4 border-amber-500 p-4 mx-4 md:mx-6 mt-4 flex items-start gap-3 shadow-sm rounded-r-md">
          ...
          {/* Page Title */}
          {title && (
            <div className="border-b bg-background/95 px-4 md:px-6 py-4">
